	...
	*/
	function user_help() {
		...
		if ( $this->subscribe2_options['email_freq'] != 'never' ) {
			...
			$screen->add_help_tab(array(
				'id' => 's2-user-help1',
				'title' => __('Overview', 'subscribe2'),
				'content' => '<p>' . __('From this page you can select your subscription preferences. You can choose the email format, which categories you would like to receive notification for and depending on the sites settings which authors you would like to read.', 'subscribe2') . '</p>'
			));
		}
	} // end user_help()
	...
	function subscribers_help() {
		...
		$screen->add_help_tab(array(
			...
			'content' => '<p>' . __('Public Subscribers are subscribers who have used the plugin form and only provided their email address.', 'subscribe2') . '</p><p>'. __('You can view and search public subscribers on this page, delete subscribers and also toggle them between Confirmed and Unconfirmed status.', 'subscribe2') . '</p>'
		));
		$screen->add_help_tab(array(
			...
			'</p><p>'. __('You can view and search registered subscribers on this page. You can delete user accounts from here with any posts created by those users being assigned to the currently logged in user. You can also apply bulk changes to user settings changing their subscription email format and categories.', 'subscribe2') . '</p>'
		));
	...
	function settings_help() {
		...
		$screen->add_help_tab(array(
			...
			'content' => '<p>' . __('This section allows you to specify settings that apply to the emails generated by the site. You can choose what types of post generate email notifications (private, password protected).', 'subscribe2') .
			'</p><p>'. __('You also specify the sender of the email on this page. You can choose Post Author or your Blogname but it is recommended to create a user account with an email address that really exists and shares the same domain name as your site (the bit after the @ should be the same as your sites web address) and then use this account.', 'subscribe2') .
			'</p><p>'. __('You can also choose whether to send email notifications at the time new posts are made (per post) or periodically with an excerpt of each post made (digest).', 'subscribe2') . '</p>'
		));
		$screen->add_help_tab(array(
			...
			'</p><p>'. __('There are special {KEYWORDS} that are used by Subscribe2 to place content into the final email but you can also type in any regular text you would like to appear in the final email', 'subscribe2') .
			'</p><p>'. __('The {KEYWORDS} are listed on the right of the templates, note that some are for per post emails only and some are for digest emails only. Make sure you use the correct keywords based upon your Email Settings.', 'subscribe2') . '</p>'
		));
		$screen->add_help_tab(array(
			'id' => 's2-settings-help4',
			'title' => __('Registered Users', 'subscribe2'),
			'content' => '<p>' . __('This section allows you to specify certain settings that apply to Registered Subscribers', 'subscribe2') .
			'</p><p>'. __('You can make certain categories compulsory so emails are always sent for posts in these categories. You can also exclude categories so that emails are not generated. Excluded categories take precedence over Compulsory categories.', 'subscribe2') .
			'</p><p>'. __('You can also define a set of default settings for new users to your site using the Auto Subscribe section. Setting specified here will be applied to any newly created user accounts while Subscribe2 is activated.', 'subscribe2') . '</p>'
		));
		$screen->add_help_tab(array(
			...
			'</p><p>'. __('You can also enable AJAX mode that is intended to work with the shortcode link parameter so that a dialog opens in the centre of the browser rather then using the regular form.', 'subscribe2') .
		...
		$screen->add_help_tab(array(
			...
			'</p><p>'. __('In the paid Subscribe2 HTML version there is also a place here to enter you license code so you can access updates automatically.', 'subscribe2') .
			'</p>'
		));
	} // end settings_help()
